...
export const GraphBox = (props) =>{
    ...
    const dragOverBox = async (event) => {
        ...
        if(!startTargetFlag){ // If you are currently holding start or target box, then check if you are also holding a wall and then update adjacency matrix accordingly
            if(wallFlag){
        ...
        }else{ // if you are currently holding start or target box, then update adjacency matrix and remove the wall tag
            setClassName(true, element);
            setWall(false)
        }
    }
    ...
    const setClassName = (flag, element) => {
        const adjMatrix = [...props.matrix.paramMatrix];
        const num = props.id;
        ...
        if(flag){
            ...
            if( num >= 0 && num <= tableWidth-1){
                if(num === 0){
                    adjMatrix[num+1][num] = 1;
                    adjMatrix[num+tableWidth][num] = 1;
            ...
            if( num >= 0 && num <= tableWidth-1){
                if(num === 0){
                    adjMatrix[num+1][num] = 0; 
                    adjMatrix[num+tableWidth][num] = 0;
                }else if (num === tableWidth - 1) {
                    adjMatrix[num-1][num] = 0;
                    adjMatrix[num+tableWidth][num] = 0;
                }else {
    ...
    return(
        <Box className='box' id={props.id} draggable onClick={changeWall} onDragStart={dragBox} onDragOver={dragOverBox} onDrop={dropBox} sx={{display:'flex', justifyContent: 'center'}}/>
    )
}